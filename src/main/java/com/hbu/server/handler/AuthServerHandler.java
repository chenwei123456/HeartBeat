...
import com.hbu.common.protobuf.Command.CommandType;
import com.hbu.common.protobuf.Message.MessageBase;
import com.hbu.server.ChannelRepository;
...
public class AuthServerHandler extends ChannelInboundHandlerAdapter {
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		...
		if(msgBase.getCmd().equals(CommandType.AUTH)){
			...
			//处理ping消息
			ctx.writeAndFlush(createData(clientId, CommandType.PING, "This is ping data").build());

		}else{
			if(ch.isOpen()){
				//触发下一个handler
				ctx.fireChannelRead(msg);
				log.info("开始进入业务入处理逻辑");
			}
		}
